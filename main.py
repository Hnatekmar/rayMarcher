...
def march(origins: torch.Tensor, directions: torch.Tensor, transform: torch.Tensor, objects: List[ObjectGroup],
          epsilon=1e-2, maximum_iterations=100):
...
def main():
    ...
    sphere_group.add_sphere([0, 0, 0], 1)
    torch.cuda.set_device(0)
    import math
    x = torch.Tensor([
       [math.sin(math.degrees(i)) * 5, math.cos(math.degrees(i)) * 5, 0, 1] for i in range(0, 360, 1)
    ])
    res = march(x, -x[:, :3], torch.eye(4, 4), [sphere_group])
    res = res[:, :2]
