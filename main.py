...
class SphereGroup(ObjectGroup):
    ...
    def nearest(self, points: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.centers.shape[0]):
            difference = torch.norm(self.centers[i, :].cuda() - points[:, :3], dim=1)
            tmp_distance = difference - self.radii[i, :].cuda()
...
def march(origins: torch.Tensor, directions: torch.Tensor, transform: torch.Tensor, objects: List[ObjectGroup],
    ...
    transformed_rays = transformed_rays.cuda()
    rows = transformed_rays.shape[0]
    distances = torch.ones(rows, 1).cuda() * float('inf')
    distances = distances
    mask = torch.zeros(rows, 1).cuda().bool()
    acc = torch.zeros_like(distances).cuda()
    if debug:
    ...
    infinity = torch.ones(rows, 1).cuda()
...
def main():
    ...
    directions = torch.zeros_like(rays).cuda()
    directions = directions[:, :3]
    directions[:, 2] = 1

    res = march(rays, directions, torch.eye(4, 4), [sphere_group], debug=False, maximum_iterations=500,
